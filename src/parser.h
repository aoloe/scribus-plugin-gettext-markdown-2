...
typedef struct {
	...
	QString *lyx_debug_pad;      /* CRC - padding to indent debugging informaiton */
} scratch_pad;
...
node * cons(node *newnode, node *list);
node * reverse_list(node *list);
void   append_list(node *newnode, node *list);

node    * mk_str_from_list(node *list, bool extra_newline);
QString * concat_string_list(node *list);
...
void   print_raw_node(QString *out, node *n);
void   print_raw_node_tree(QString *out, node*n);
